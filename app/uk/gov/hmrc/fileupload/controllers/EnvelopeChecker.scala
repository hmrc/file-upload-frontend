    ...
    Iteratee.flatten {
      check(envelopeId).map {
        ...
        case Xor.Right(otherStatus) =>
          Done(Locked(msg(s"Unable to upload to envelope: $envelopeId with status: $otherStatus")))
        case Xor.Left(EnvelopeStatusNotFoundError(_)) =>
          Done(NotFound(msg(s"Unable to upload to nonexistent envelope: $envelopeId")))
        case Xor.Left(error) =>
